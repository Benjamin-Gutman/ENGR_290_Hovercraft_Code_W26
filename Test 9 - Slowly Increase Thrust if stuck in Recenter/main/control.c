...
static uint8_t i_recenter = 0;
static uint8_t i_prepare = 0;

/*=====Thrust Unsticker variables=====*/

static int recenter_thrust = THRUST_RECENTER;
static unsigned long last_thrust_bump = 0;
...
static float clamp_servo(float angle) {
   ...
   if (angle > SERVO_RIGHT_MAX_PD) {
        return SERVO_RIGHT_MAX_PD;
    }
    if (angle < SERVO_LEFT_MAX_PD) {
        return SERVO_LEFT_MAX_PD;
    }
...
void apply_control(void) {
        ...
        if (state == STATE_RECENTER) {
        i_recenter = 0;   // reset ONLY when entering recenter
        }

        if (state == STATE_PREPARE_TURN) {
        i_prepare = 0;  //Restet
    ...
    switch (state) { //State is a global variable from state_machine.c
        ...
        {
            if (i_prepare < 70) {
        // STOP phase (short pause before turn)
            set_lift(0);
            thrust_command = 0;
       ...
       /*--------------------------------------------------*/
        ...
        /*--------------------------------------------------*/
        {
           ...
           /* Original Recenter Code*/
              /*if (i_recenter < 80) {
                    // STOP phase
                    set_lift(0);
                    thrust_command = 0;
                    servo_command = 0;   // or keep last value if you prefer

                    i_recenter++;
                } else {
                    // NORMAL recenter behavior
                    set_lift(100);
                    thrust_command = THRUST_RECENTER;
                    servo_command = yaw_pd_control(yaw_target, yaw_deg);
                    // FAIL to recenter behavior
                    if (yaw_rate_dps <= 1)
                    {
                        thrust_command = thrust_command + 5;
            ...
            /*Unstick in recenter version*/
            
            
            {
                const float YAW_RATE_MIN_DPS = 1.0f;
                const unsigned long BUMP_INTERVAL_MS = 300;
                const int THRUST_STEP = 5;
                const int THRUST_RECENTER_MAX = 90;
                ...
                if (i_recenter < 80) {
                    ...
                    recenter_thrust = THRUST_RECENTER;
                    last_thrust_bump = millis();
                } 
                else {
                    float yaw_error = yaw_target - yaw_deg;
                    ...
                    // If craft is turning the wrong way, or barely turning at all,
                    // slowly increase thrust
                    if ((yaw_error * yaw_rate_dps < 0) || (fabs(yaw_rate_dps) < YAW_RATE_MIN_DPS)) {
                        if (millis() - last_thrust_bump >= BUMP_INTERVAL_MS) {
                            recenter_thrust += THRUST_STEP;

                            if (recenter_thrust > THRUST_RECENTER_MAX) {
                                recenter_thrust = THRUST_RECENTER_MAX;
                            }
                    ...
                    thrust_command = recenter_thrust;
                }
